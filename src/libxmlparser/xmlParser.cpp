 ...
 * changed by eQ-3 GmbH 2006-2007
 *
 * NOTE:
 *
...
LPTSTR fromXMLString(LPCTSTR s, int lo)
{
    ...
    while (((lo--)>0)&&(*s))
    {
        if (*s==_T('&'))
        {
            ...
            else if (_tcsnicmp(s,_T("gt;"  ),3)==0) { s+=3; lo-=3; }
            else if (_tcsnicmp(s,_T("amp;" ),4)==0) { s+=4; lo-=4; }
            else if (_tcsnicmp(s,_T("apos;"),5)==0) { s+=5; lo-=5; }
            else if (_tcsnicmp(s,_T("quot;"),5)==0) { s+=5; lo-=5; }
		}else{
			s++;
		}
		ll++;
    ...
    while (ll--)
    {
        if (*ss==_T('&'))
        {
                 if (_tcsnicmp(ss+1,_T("lt;"  ),3)==0) { *(d++)=_T('<' ); ss+=4; }
            else if (_tcsnicmp(ss+1,_T("gt;"  ),3)==0) { *(d++)=_T('>' ); ss+=4; }
            else if (_tcsnicmp(ss+1,_T("amp;" ),4)==0) { *(d++)=_T('&' ); ss+=5; }
            else if (_tcsnicmp(ss+1,_T("apos;"),5)==0) { *(d++)=_T('\''); ss+=6; }
            else if (_tcsnicmp(ss+1,_T("quot;"),5)==0) { *(d++)=_T('"' ); ss+=6; }
            else { *(d++)=*ss; ss++; }
        } else { *(d++)=*ss; ss++; }
    }
    *d=0;
...
static XMLNextToken GetNextToken(XML *pXML, int *pcbToken, enum TokenTypeTag *pType)
{
    XMLNextToken        result;
    LPCTSTR          lpXML;
    TCHAR            ch;
...
void XMLNode::setName(LPCTSTR lpszName)
{
	if (d->lpszName) free((void*)d->lpszName);
	//d->lpszName=(char*)malloc(strlen(lpszName)+1)
	d->lpszName=lpszName;
}

void XMLNode::setNameConst(LPCTSTR lpszName)
{
	if(!lpszName)
		return;
	char *cTemp;
...
// private:
XMLNode::XMLNode(XMLNodeData *pParent, LPCTSTR lpszName, int isDeclaration)
{
...
XMLAttribute *XMLNode::addAttributeConst(LPCTSTR lpszName, LPCTSTR lpszValuev)
{
...
{
    XML *pXML=(XML *)px;
    XMLClearTag *pClear=(XMLClearTag *)pa;
    int cbTemp = 0;
    LPCTSTR lpszTemp;
...
//	printf("%s::getChildNode(%s)\n", d->lpszName?d->lpszName:"", name);
    int i=0,n=d->nChild;
    if (j) i=*j;
...
{
    if (!d) return NULL;
//	printf("%s::getAttribute(%s)\n", d->lpszName?d->lpszName:"", lpszAttrib);
